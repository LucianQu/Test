...
public class ClockView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs){
        ...
        colorDialLower = typedArray.getColor(R.styleable.ClockView_color_dial_lower, DEFAULT_COLOR_LOWER);
        colorDialMiddle = typedArray.getColor(R.styleable.ClockView_color_dial_middle, DEFAULT_COLOR_MIDDLE);
        colorDialHigh = typedArray.getColor(R.styleable.ClockView_color_dial_high, DEFAULT_COLOR_HIGH);
        textSizeDial = (int) typedArray.getDimension(R.styleable.ClockView_text_size_dial, sp2px(DEFAULT_TEXT_SIZE_DIAL));
        strokeWidthDial = (int) typedArray.getDimension(R.styleable.ClockView_stroke_width_dial, dp2px(DEFAULT_STROKE_WIDTH));
        radiusDial = (int) typedArray.getDimension(R.styleable.ClockView_radius_circle_dial, dp2px(DEFAULT_RADIUS_DIAL));
        titleDial = typedArray.getString(R.styleable.ClockView_text_title_dial);
        titleDialSize = (int) typedArray.getDimension(R.styleable.ClockView_text_title_size, dp2px(DEAFAULT_TITLE_SIZE));
        titleDialColor = typedArray.getColor(R.styleable.ClockView_text_title_color, DEAFAULT_COLOR_TITLE);
        valueTextSize = (int) typedArray.getDimension(R.styleable.ClockView_text_size_value, dp2px(DEFAULT_VALUE_SIZE));
        animPlayTime = typedArray.getInt(R.styleable.ClockView_animator_play_time, DEFAULT_ANIM_PLAY_TIME);
        //typearray是一个共享的资源，使用完毕必须回收它
        typedArray.recycle();
    }
    ...
    * */
    ...
    private float beforValue = 0; // 保存之前的数值（这里是新添的）
    public void setCompleteDegree(float degree){
        currentValue = degree ;
        //ValueAnimator animator = ValueAnimator.ofFloat(beforValue, degree);
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                Log.e("lucian_current", currentValue+"") ;
                invalidate();
            }
        });
